...
public class FileSenderFrame extends JFrame {
	...
	class FileChooseActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			FileNameExtensionFilter filter = new FileNameExtensionFilter("JPG & GIF & PNG Images", // 파일
																									// 이름에
																									// 창에
																									// 출력될
																									// 문자열
					"jpg", "gif", "png"); // 파일 필터로 사용되는 확장자. *.jpg. *.gif,
											// *.png만 나열됨
			...
			if (ret != JFileChooser.APPROVE_OPTION) { // 사용자가 창을 강제로 닫았거나 취소 버튼을
														// 누른 경우
				JOptionPane.showMessageDialog(FileSenderFrame.this, "파일을 선택하지 않았습니다", "경고",
	...
	class MyActionListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				fout = new BufferedOutputStream(socket.getOutputStream()); // 서버로의
																			// 출력
																			// 스트림
